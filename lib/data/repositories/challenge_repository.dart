...
class ChallengeRepository {
  ...
  Future<ChallengeListModel?> getChallengeList(String type, String keyword, int page, int size, String sort) async {
    try {
      final queryParams = {
        'type': type,
        'keyword': keyword,
  ...
  Future<String> addChallenge(String title, String description, String type, int betPoint, int minutes, String start, String end) async {
    try {
      final body = {
  ...
  Future<int?> getMyChallengeId() async {
    try {
      ...
      print('my 챌린지 id 가져오기 실패: ${response.statusCode} ${jsonDecode(response.body)}');
      return null;
